		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <Notification>");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "ManageNotificationListSet",
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Notification") === oObject.Notification;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Notification;
									} else {
										sCurrentId = aItemsNotInTheList[0].Notification;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ManageNotificationListSet", {
										Notification : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Notification"),
						title: oBindingContext.getProperty("Notification")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<ManageNotificationListSet> (0)"}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("Notification");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all ManageNotificationListSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<ManageNotificationListSet> (" + iExpectedLength + ")"}),
								success : function () {
